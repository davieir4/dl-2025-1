...
class Perceptron:
    ...
    def predict(self, X):
        ...
        X_bias = np.hstack([np.ones((X.shape[0], 1)), X])  # adiciona coluna de 1s (bias)
        linear_output = X_bias @ self.weights  # produto escalar (shape: [n amostras])
    ...
    def fit(self, X, y):
        ...
        X_bias = np.hstack([np.ones((X.shape[0], 1)), X])  # adiciona coluna de bias

        for epoch in range(self.epochs):
            for xi, yi in zip(X_bias, y):
                y_pred = self.activation(np.dot(xi, self.weights))
                error = yi - y_pred
